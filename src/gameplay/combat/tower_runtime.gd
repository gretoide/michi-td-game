...
var can_detect_invisible := false
...
func _abilities_for(definition: GemDefinition) -> PackedStringArray:
	...
	if not display_ability.is_empty():
		var key := _ability_key(display_ability)
		if key not in result: result.append(key)
	return result

func _ability_key(value: String) -> String:
	var parts: Array = Array(value.to_lower().strip_edges().replace("-", "_").replace(" ", "_").split("_", false))
	if not parts.is_empty() and parts[-1].is_valid_int(): parts.pop_back()
	return "_".join(parts)
...
func attack(target: EnemyRuntime) -> bool:
	...
	return targeting.set_manual(target, position, stats.range_units, can_detect_invisible)
...
func tick(delta: float, enemies: Array[EnemyRuntime], projectile_speed := 1000.0) -> HomingProjectile:
	...
	var target := targeting.tick(position, stats.range_units, enemies, can_detect_invisible)
