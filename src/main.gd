...
func _build_ui() -> void:
    ...
    parchment_brightener = ColorRect.new(); parchment_brightener.color = Color(1.0,0.94,0.80,0.30); parchment_brightener.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT); parchment_brightener.offset_left = 34; parchment_brightener.offset_top = 34; parchment_brightener.offset_right = -34; parchment_brightener.offset_bottom = -34; parchment_brightener.mouse_filter = Control.MOUSE_FILTER_IGNORE; parchment_brightener.visible = false; parchment_panel.add_child(parchment_brightener)
...
func _show_verification() -> void:
    access_state = AccessState.VERIFY_EMAIL; glass_panel.custom_minimum_size = Vector2(640,520); parchment_brightener.visible = true; root_ui.visible = false; back_button.visible = true; verification = EmailVerificationView.new(); verification.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT); verification.setup(verification_email); verification.verify_requested.connect(_verify_code); verification.resend_requested.connect(_resend_code); parchment_panel.add_child(verification)
